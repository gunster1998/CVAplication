  ...
  const renderExperiance = resum.Experience.map((job)=> (
    <div key={job.id}>
      <div className={styles.ExperianceDate}>
        ...
        <p className={styles.ExperianceDateStart}>Дата начала работы: {job.StartDate}</p>
        <p className={styles.ExperianceDateEnd}>Дата окончания работы: {job.EndDate}</p>
        <p>{job.Description}</p>
      </div>
    </div>
  ))
        ...
        <div className={styles.resumeRenderA4Right}>
          <div className={styles.A4RightTop}>
            <div className={styles.A4RightTopName}>
              <p className={styles.A4RightTopNameText}>{resum.FirstName} {resum.LastName} </p>
            </div>
            <div className={styles.A4RightProfession}>
              <p>{resum.Profession}</p>
            </div>
            <div className={styles.A4RightEmailandAdress}>
              <div className={styles.A4RightEmail}><img className={styles.A4RightImg} src={iconEmail} alt="" /> <p className={styles.A4RightEmailText}>{resum.Email}</p></div>
              <div className={styles.A4RightAdress}><img className={styles.A4RightImg} src={iconAdress} alt="" />{resum.Counrty} Г. {resum.City}</div>
            </div>
            <div className={styles.A4RightPhone}><img className={styles.A4RightImg} src={iconPhone} alt="" />{resum.Phone}</div>
          </div>
